...
class IdbRejectReason(models.TransientModel):
    ...
    # sample_management_id = fields.Many2one('mrp.eco', string='样品管理')

    def submit_reject_reason(self):
        sample_management_id = self.env['mrp.eco'].browse(self.env.context.get('id'))
        # 返回上级状态
        state = {
...
class MrpEcoStage(models.Model):
    ...
    # domain不能是当前记录的id，并且不能是当前记录的下级记录的id
    superiors_id = fields.Many2one('mrp.eco.stage', string='Superior stage')
    junior_id = fields.Many2one('mrp.eco.stage', string='Junior stage')
...
class MrpEco(models.Model):
    ...
    # 产品分类
    category_id = fields.Many2one('product.category', string='Product classification', required=True)
    ...
    def copy(self, default=None):
        ...
        default['name'] = self.env['ir.sequence'].next_by_code('mrp.eco')
        default['state'] = 'draft'
        default['factory_number'] = self._get_factory_number(self.category_id)
        return super(MrpEco, self).copy(default)
    ...
    #         vals['name'] = self.env['ir.sequence'].next_by_code('mrp.eco') or _("New")
    #     return super(MrpEco, self).create(vals_list)

    @api.model
    def default_get(self, field_list):
        res = super(MrpEco, self).default_get(field_list)
        # 获取默认模板
    ...
    def _compute_purchase_count(self):
        for record in self:
            record.purchase_count = self.env['purchase.order.line'].search_count(
                [('order_id.sample_management_id', '=', record.id)])
    ...
    def general_material_preparation(self):
        if self.bom_count <= 0:
            raise UserError(
                _('Product BOM not created, do not submit next step! Please perform ET data generation BOM operation!'))
            # 检查部位资料是否填写产品
        for bom_line_id in self.bom_id.bom_line_ids:
            if not bom_line_id.product_id:
                raise UserError(
                    _('Position information and products are not filled in, do not submit the next step!'))
        # 检查无部位资料是否填写产品
        for no_bom_line_id in self.bom_id.no_bom_line_ids:
            if not no_bom_line_id.product_id:
                raise UserError(
                    _('No part information and products are not filled in, do not submit the next step!'))
    ...
    # 客户评语
    customer_comment_ids = fields.One2many('idb.customer.comment', 'sample_management_id',
    ...
    def approve(self):
        ...
        junior_id = self.stage_id.junior_id
        if not junior_id:
            raise UserError(_('The current stage has no lower stage, Contact the administrator!'))
        self.stage_id = junior_id.id
        return res

    def reject(self):
        # 审核拒绝返回上级阶段
        res = super(MrpEco, self).reject()
        superiors_id = self.stage_id.superiors_id
        if not superiors_id:
            raise UserError(_('The current stage has no higher stage, Contact the administrator!'))
        self.stage_id = superiors_id.id
        return res
